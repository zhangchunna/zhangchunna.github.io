...
1.2  lastIndexOf()  指定位置开始向前查找,最后出现的位置
...
2.1  substr(start,length) length:可选,默认为长度 
...
2.2 substring(start,end) end:不包括end
...
2.3  slice(start,end) 提取一个字符串的一部分，并返回一新的字符串 end不包括
...
5.1 split(start,end) 将字符串拆分为一串数组
...
5.2 replace (regexp/substr, newSubStr|function)
...
5.4 match(regexp) 找到一个或多个正则表达式的匹配
